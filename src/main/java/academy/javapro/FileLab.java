...
public class FileLab {
	 ...
	 */
	private static void addSalesRecord(String orderId, String date, String customerName, String productId,
			String quantity, String unitPrice, String totalAmount, String region) {
	...
	/**
	 * Display the data in a formatted table Using enhanced for loop
	 */
	private static void displayData(List<Map<String, String>> data) {
		// Display header
		System.out.println(String.format("%-8s %-12s %-20s %-10s %-8s %-10s %-12s %-8s", "OrderID", "Date",
				"CustomerName", "ProductID", "Quantity", "UnitPrice", "TotalAmount", "Region"));
		System.out.println("-".repeat(90));

		// Display each record using enhanced for loop
		for (Map<String, String> record : data) {
			System.out.printf("%-8s %-12s %-20s %-10s %-8s %-10s %-12s %-8s\n", record.get("OrderID"),
					record.get("Date"), record.get("CustomerName"), record.get("ProductID"), record.get("Quantity"),
	...
	/**
	 * Filter 1: Find orders with quantity greater than the specified value Using
	 * enhanced for loop and continue statement for flow control
	 */
	private static List<Map<String, String>> filterQuantityGreaterThan(List<Map<String, String>> data,
			int minQuantity) {
	...
	/**
	 * Filter 2: Find orders with unit price greater than the specified value Using
	 * traditional while loop instead of for loop
	 */
	private static List<Map<String, String>> filterUnitPriceGreaterThan(List<Map<String, String>> data,
			double minPrice) {
	...
	/**
	 * Filter 3: Find orders from a specific region Using enhanced for loop with
	 * switch statement for region comparison
	 */
	private static List<Map<String, String>> filterByRegion(List<Map<String, String>> data, String targetRegion) {
		...
		for (Map<String, String> record : data) {
			...
			switch (recordRegion.toLowerCase()) {
			case "north":
			case "south":
			case "east":
			case "west":
